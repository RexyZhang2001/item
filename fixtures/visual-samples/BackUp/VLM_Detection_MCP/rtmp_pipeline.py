...
def _jpeg_quality() -> int:
    return int(os.environ.get("VLM_JPEG_QUALITY", "70"))


def _jpeg_bytes(bgr) -> bytes:
    q = _jpeg_quality()
...
def _jpeg_b64(bgr) -> str:
    return base64.b64encode(_jpeg_bytes(bgr)).decode("ascii")
...
def analyze_rtmp_stream_bgr(stream_url: str) -> tuple[Any, Any, Any, dict[str, Any]]:
    """抽流 + YOLO，返回三张 BGR 图与元数据。失败时抛出 RuntimeError。"""
    ...
    return original, human_vis, machine_vis, meta


def analyze_rtmp_stream(stream_url: str, *, return_urls: bool = False) -> dict[str, Any]:
    """输入 RTMP/HLS/FLV 地址字符串，返回固定顺序三张图（JSON 含 base64）。
    ...
    """
    try:
        original, human_vis, machine_vis, meta = analyze_rtmp_stream_bgr(stream_url)
    except RuntimeError as e:
        return {"ok": False, "error": str(e)}

    b1, b2, b3 = _jpeg_b64(original), _jpeg_b64(human_vis), _jpeg_b64(machine_vis)

    include_detections = os.environ.get("MCP_INCLUDE_DETECTIONS", "0").strip() in (
        "1",
        "true",
        "yes",
    )
    omit_dup_b64 = os.environ.get("MCP_OMIT_DUPLICATE_B64", "1").strip() in (
        "1",
        "true",
        "yes",
    )
    ...
    if include_detections:
        out["detections"] = {"worker": meta["workers"], "machinery": meta["machinery"]}
    if not omit_dup_b64:
        out["image"] = b1
        out["image_human"] = b2
        out["image_machine"] = b3
